...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select * from cadastro where loginU='${login}' and senha='${senha}'`);
  }).then(consulta => {
...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select count(*) as contagem from cadastro where loginU = '${login}'`);
  }).then(consulta => {
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into cadastro (nome, sobrenome, loginU, senha) values ('${nome}','${sobrenome}','${login}','${senha}')`).then(function() {
